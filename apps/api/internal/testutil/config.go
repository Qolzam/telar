...
// Falls back to individual environment variables if DSN is not available.
...
// LoadTestConfig loads configuration from environment with safe fallbacks.
// Generates ephemeral secrets if not provided to avoid hardcoded secrets in code.
// If global config is available, uses those values to ensure consistency.
func LoadTestConfig() (*TestConfig, error) {
	// Generate RSA keys for better JWT library compatibility
	pubPEM, privPEM := GenerateECDSAKeyPairPEM(&testing.T{})

	// Parse PostgreSQL DSN if available, otherwise use individual env vars
	...
	// Note: testutil config is independent of global config to ensure test isolation
...
func (c *TestConfig) ToServiceConfig(dbType string) *platform.ServiceConfig {
	...
	if dbType == dbi.DatabaseTypeMongoDB {
		sc.MongoConfig = &dbi.MongoDBConfig{
			...
			Port: 27017,
			// --- THE FIX ---
			MaxPoolSize:    200, // Increased from 100 to handle high concurrency
			MinPoolSize:    10,  // Set a minimum to keep warm connections available
			ConnectTimeout: 30,  // Keep a reasonable connection timeout
			SocketTimeout:  60,
			MaxIdleTime:    300,
			// Add a server selection timeout to fail faster if the pool is truly exhausted
			ServerSelectionTimeout: 5, // 5 seconds
		}
		...
		sc.PostgreSQLConfig = &dbi.PostgreSQLConfig{
			...
			SSLMode:            "disable",
			Schema:             c.PGSchema,
			ConnectTimeout:     30,  // Added connection timeout
			MaxOpenConnections: 50,  // Increased for better concurrency
			MaxIdleConnections: 10,  // Added idle connections for faster reuse
...
func getBoolEnv(key string, defVal bool) bool {
	if v, ok := os.LookupEnv(key); ok {
		...
		// For invalid boolean values, return default value
	}
	return defVal
}
