...
// NewProfileRepositoryFromConfig creates a ProfileRepository from platform config
// This is a helper function for wiring up the new domain-specific repository
func NewProfileRepositoryFromConfig(ctx context.Context, cfg *platformconfig.Config) (repository.ProfileRepository, error) {
	if cfg == nil {
		return nil, fmt.Errorf("config is required")
	}

	if cfg.Database.Type != "postgresql" {
		return nil, fmt.Errorf("only PostgreSQL is supported for profiles repository")
	}
	...
	if pgConfig.Schema != "" {
		if err := repository.ApplyProfilesMigration(ctx, client, pgConfig.Schema); err != nil {
			client.Close()
			return nil, fmt.Errorf("failed to apply profiles migration: %w", err)
		}
	}

	return repository.NewPostgresProfileRepository(client), nil
}
